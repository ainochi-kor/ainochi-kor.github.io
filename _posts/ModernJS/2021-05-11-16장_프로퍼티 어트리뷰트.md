...
layout: post
author: ainochi-kor
title: Modern JS/16장 프로퍼티 어트리뷰트
---
...
const person = {
  ...
  // fullName은 접근자 함수를 구성된 접근자 프로퍼티다.
  // getter 함수
  get fullName() {
...
// 접근자 프로퍼티 fullName에 getter 함수가 호출된다.
...
- 객체의 프로퍼티나 메서드에 접근하려고 할 때 행당 객체에 접근하려는 프로퍼티 또는 메서드가 없다면 프로퍼티 체인을 따라 프로토타입의 프로퍼티나 메서드를 차례대로 검색한다.
...
// {value: {...}, writable: true: true, enumerable: false, configurable: false }
```
...
// Uncaught TypeError: Cannot radefine property: lastName
...
---------|------------------|----|----|----|----|-----|
객체 확장 금지|Object.prevenmtExtensions|X|O|O|O|O|
객체 밀봉|Object.seal|X|X|O|O|X|
객체 동경|Object.freeze|X|X|O|X|X|
...
Object.preventExtenstions(person);
...
- 객체 밀봉(seal)이란 프로퍼티 추가 및 삭제와 프로퍼티 어트리뷰트 재정의 금지를 의미.
...
// 프로퍼티 추가가 금지된다.
person.age = 20; // 무시. strict model에서는 에러
console.log(person); // {name: "Lee"}
...
Object.defineProperty(person, 'name', {configuration: true});
// TypeError: Cannot redefine property: name
```

### 16.5.3 객체 동결
- Object.freeze 메서드는 객체를 동결한다. 
- 객체 동결이란 프로퍼티 추가 및 삭제와 프로퍼티 어트리뷰트 재정의 금지, 프로퍼티 값 갱신 금지를 의마한다.
- **즉, 동결된 객체는 읽기만 가능하다.**
...
// 중첩 객체까지 동결하지 못한다.
console.log(Object.isFrozne(person.address)) // false
...
function deepFreeze(target) {
  ...
  if(target && typeof target === 'object' && !Object.is(target)) {
